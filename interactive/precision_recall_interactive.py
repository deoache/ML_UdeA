...
def precision_recall_viz(threshold=0):
    ...
    positive_class = pd.DataFrame(
        dict(x=np.linspace(-3, 10, n), y=np.ones(n, dtype=int))
    )
    negative_class = pd.DataFrame(
        dict(x=np.linspace(-10, 3, n), y=np.zeros(n, dtype=int))
    )

    plt.figure(figsize=(12, 7))
    sns.scatterplot(x=positive_class.x, y=0.2 * np.ones(n), label="Positive class")
    sns.scatterplot(x=negative_class.x, y=-0.2 * np.zeros(n), label="Negative class")
